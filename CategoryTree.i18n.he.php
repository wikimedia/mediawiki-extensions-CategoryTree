<?php

/**
 * Internationalisation file for the CategoryTree extension
 *
 * @addtogroup Extensions
 */

$messages['categorytree']= 'עץ קטגוריות';
$messages['categorytree-tab']= 'עץ';
$messages['categorytree-header']= 
'הקלידו שם קטגוריה כדי לראות את תכניה במבנה עץ. שימו לב שהדבר דורש תכונת JavaScript מתקדמת, הידועה בשם AJAX. אם יש לכם דפדפן ישן מאוד, או ש־JavaScript מנוטרלת אצלכם בדפדפן, הוא לא יעבוד.
';

$messages['categorytree-category']= 'קטגוריה';
$messages['categorytree-go']= 'הצג את העץ';
$messages['categorytree-parents']= 'הורים';

$messages['categorytree-mode-categories']= 'קטגוריות בלבד';
$messages['categorytree-mode-pages']= 'דפים שאינם תמונות';
$messages['categorytree-mode-all']= 'כל הדפים';

$messages['categorytree-collapse']= 'כיווץ';
$messages['categorytree-expand']= 'הרחבה';
$messages['categorytree-load']= 'טעינה';
$messages['categorytree-loading']= 'בטעינה';
$messages['categorytree-nothing-found']= 'לא נמצאו תוצאות';
$messages['categorytree-no-subcategories']= 'אין קטגוריות משנה';
$messages['categorytree-no-pages']= 'אין דפים או קטגוריות משנה';
$messages['categorytree-not-found']= "הקטגוריה '''$1''' לא נמצאה";
$messages['categorytree-error'] = 'בעיה בטעינת המידע.';
$messages['categorytree-retry'] = 'אנא המתינו מעט ונסו שנית.';

$messages['categorytree-show-list'] = "תצוגת רשימה";
$messages['categorytree-show-tree'] = "תצוגת עץ";
$messages['categorytree-too-many-subcats'] = "לא ניתן להציג את קטגוריות המשנה כעץ כיוון שהן מרובות מדי.";


